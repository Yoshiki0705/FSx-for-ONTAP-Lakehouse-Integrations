...
| Q2 | AWS | **Escalated 2026-08-12.** Object tag keys/values at U+0100 and above are rejected with `InvalidTag` for most strings but accepted for some (`分類` accepted, `東京` rejected — both two-character CJK). AWS Support reviewed it against the S3 and FSx for ONTAP documentation, found no intentional validation that matches, and filed it with the FSx for NetApp ONTAP service team as a potential defect in the tag validation layer. Follow-up added three eliminations: validation is **not** per-character (a character rejected alone can be accepted inside a longer string), not order-dependent, and not position-dependent. ASCII remains the guidance until a determination arrives |
